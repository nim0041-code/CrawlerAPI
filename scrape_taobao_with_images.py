...
def download_product_images(
    ...
    root = image_output_root or IMAGE_OUTPUT_ROOT
    output_dir = Path(root) / platform / safe_product_id
...
def scrape_product(
    ...
    selected_auth_state = auth_state_path or AUTH_STATE
    selected_image_output_root = image_output_root or IMAGE_OUTPUT_ROOT
    auth_state_exists = Path(selected_auth_state).exists()
    ...
    with sync_playwright() as p:
        ...
        if auth_state_exists:
            context = browser.new_context(storage_state=selected_auth_state)
        else:
        ...
        try:
            ...
            if not auth_state_exists:
                return {
                    ...
                    "auth_state_path": selected_auth_state,
            ...
            if title in ["登录", ""] and not price:
                return {
                    ...
                    "message": "未能提取到有效商品标题和价格，可能仍在登录页、验证页，或页面结构发生变化。",
                    "product_id": product_id,
                    "platform": platform,
                    "current_url": page.url,
                    "page_title": page.title(),
            ...
            result = {
                ...
                "raw_html_path": raw_html_path,
                "auth_state_path": selected_auth_state,
                "image_output_root": selected_image_output_root,
            }

            if download_images:
                image_candidates = extract_image_candidates(page)
                image_result = download_product_images(
                    image_candidates,
                    platform,
                    product_id,
                    image_output_root=selected_image_output_root,
